(function() {
	...
	//添加节点到点击节点的子节点后
	function addNode(currentChoose) {
		var val = document.querySelectorAll('.control input')[1].value.trim(),
		...
		currentChoose.appendChild(elDiv);
	}
	...
	var handleClick = function( ev ) {
		switch(ev.target.className) {
			...
			case 'query' :
				...
				break;
			case 'delete' :
				deleteAll(currentChoose);
				break;
			case 'add' :
				addNode(currentChoose);
				break;
			default :
				console.log('无效点击');
		}
	};
	var currentClick = function( ev ) {
		if (currentChoose) {
			currentChoose.dataset.isBeingOrder = '';
		}
		ev.target.dataset.isBeingOrder = 'choose';
		currentChoose = ev.target;
	};
	controler.addEventListener('click', handleClick, false);
	rootNode.addEventListener('click', currentClick, false);
})()